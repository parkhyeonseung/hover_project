...
from hover_joy.msg import stringarraymsg
...
class joystick:
    def __init__(self):
        self.hover = Ardu(port = "/dev/ttyMEGA")
        self.sp = '00'
        self.st = '00'
        self.x = None
        self.y = None
    ...
    def yolocallback(self,xywh):
        xywh = xywh.data
        if (self.yolo == True) and (not self.permission) and (not self.emergency):
            ...
            if xywh[0] == 10.:
                ...
                # self.yolo_data.data = [self.sp,self.st]
                # self.yolo_joy.publish(self.yolo_data)
        elif (self.yolo == True) and (self.permission) and (not self.emergency) :
            self.hover.input('s0')
        elif self.yolo == True:
            self.hover.input('00')
