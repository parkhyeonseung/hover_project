...
hover_port = rospy.get_param('hover_port','/dev/ttyMEGA')
imu_port = rospy.get_param('imu_port','/dev/ttyIMU')
os.system(su_port+hover_port)
os.system(su_port+imu_port)
if rospy.get_param('hover','hover') == 'hover':
    print('mode hover')
    hover = hover_app(port = hover_port)
    hover_b = True
elif rospy.get_param('hover','hover') == 'debug':
    print('mode debug')
    g_debug = True


# 小车底层处理库
g_bot = Rosmaster(com = imu_port,debug=g_debug)
# 启动线程接收串口数据
...
if __name__ == '__main__':
    rospy.init_node('rosmaster_main')
